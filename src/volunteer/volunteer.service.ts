...
export class VolunteerService {
  ...
  async signup(dto: CreateVolunteerDto) {
    ...
    const existingUsername = await this.volunteerRepo.findOne({ where: { username: dto.username } });
    if (existingUsername) {
    ...
    const token = jwt.sign(
      { userId: savedUser.id, role: savedUser.role, volunteerId: savedVolunteer.id },
  ...
  async verifyOtp(dto: VerifyOtpDto) {
    const volunteer = await this.findVolunteerByEmail(dto.email, { user: true });
    if (!volunteer?.user) {
      throw new UnauthorizedException('Invalid OTP');
    }

    const otp = await this.otpRepo.findOne({
      where: { user: { id: volunteer.user.id }, purpose: OtpPurpose.SIGNUP, isUsed: false },
      relations: { user: true },
  ...
  async login(dto: LoginDto) {
    ...
    const isPasswordValid = await bcrypt.compare(dto.password, volunteer.password);
    if (!isPasswordValid) {
      throw new UnauthorizedException('Invalid credentials');
    }

    const token = jwt.sign(
      { userId: volunteer.user.id, role: volunteer.user.role, volunteerId: volunteer.id },
      process.env.JWT_SECRET ?? 'dev-secret',
      { expiresIn: '1d' },
    );
  ...
  async verifyLoginOtp(dto: VerifyOtpDto) {
    ...
    const otp = await this.otpRepo.findOne({
      where: { user: { id: volunteer.user.id }, purpose: OtpPurpose.LOGIN, isUsed: false },
      relations: { user: true },
      order: { createdAt: 'DESC' },
    });
  ...
  async getProfile(volunteerId: number) {
    const volunteer = await this.findVolunteerByIdentity(volunteerId, { skills: true, user: true });
    if (!volunteer) {
      throw new NotFoundException('Volunteer not found');
    }
    return volunteer;
  ...
  async addSkill(volunteerId: number, skillId: number) {
    const volunteer = await this.findVolunteerByIdentity(volunteerId, { skills: true });
    if (!volunteer) {
      throw new NotFoundException('Volunteer not found');
    }
  ...
  async apply(volunteerId: number, dto: ApplyTaskDto) {
    ...
    const volunteerCall = await this.volunteerCallRepo.findOne({ where: { id: dto.volunteerCallId } });
    if (!volunteerCall) {
  ...
  async createWorkLog(volunteerId: number, dto: WorkLogDto) {
    ...
    if (assignment.application?.volunteer?.id !== volunteer.id) {
      throw new UnauthorizedException('Assignment does not belong to this volunteer');
    }
  ...
  async getWorkLogs(volunteerId: number, assignmentId?: number, from?: string, to?: string) {
    const volunteer = await this.findVolunteerByIdentity(volunteerId);
    if (!volunteer) {
      throw new NotFoundException('Volunteer not found');
    }
