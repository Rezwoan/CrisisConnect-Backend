...
export class JwtAuthGuard implements CanActivate {
  ...
  private extractToken(
    req: Request & { user?: any; cookies?: Record<string, string> },
  ): string | undefined {
    const headerValue =
  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    ...
    const token = this.extractToken(
      req as Request & { user?: any; cookies?: Record<string, string> },
    );
    ...
    try {
      ...
      const volunteerId = Number(
        payload.volunteerId ?? payload.userId ?? payload.id ?? payload.sub,
      );

      if (role !== 'VOLUNTEER') {
        throw new ForbiddenException(
          'Only volunteer accounts can access this route',
        );
      }
